...
namespace Reality.Game.Items.DefaultBehaviorHandlers
{
    public static class ArrowHandler
    {
        ...
        {
            ItemEventDispatcher.RegisterEventHandler(ItemBehavior.Arrow, new ItemEventHandler(HandleArrow));
        }

        private static bool HandleArrow(Session Session, Item Item, RoomInstance Instance, ItemEventType Event, int RequestData, uint Opcode)
        {
            ...
            bool mIsInFront = (Actor.Position.X == Item.SquareInFront.X && Actor.Position.Y == Item.SquareInFront.Y);
            bool mIsInTele = !mIsInFront && (Actor.Position.X == Item.RoomPosition.X &&
