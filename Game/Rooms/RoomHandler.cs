...
namespace Reality.Game.Rooms
{
    class RoomHandler
    {
        ...
        public static void Initialize()
        {
            ...
            // GROUPS
            DataRouter.RegisterHandler(230, new ProcessRequestCallback(GetGroupBadges));
            DataRouter.RegisterHandler(231, new ProcessRequestCallback(SendGroupData));
            DataRouter.RegisterHandler(3257, new ProcessRequestCallback(SendGroupAbout));
            DataRouter.RegisterHandler(3260, new ProcessRequestCallback(SendMyGroup));

            //WIRED
            //WIRED - SAVING
            DataRouter.RegisterHandler(3051, new ProcessRequestCallback(SaveWired)); //Save: Effect
            DataRouter.RegisterHandler(3050, new ProcessRequestCallback(SaveWired)); //Save: Trigger
            DataRouter.RegisterHandler(3052, new ProcessRequestCallback(SaveWired)); //Save: Conditions
        }
        ...
        public static void MoveUserToPos(Session Session, int X, int Y)
        {
            RoomInstance Instance = RoomManager.GetInstanceByRoomId(Session.CurrentRoomId);
            ...
            Actor.MoveToPos(new Vector2(X, Y), false);
        }
        ...
        private static void SendGroupData(Session Session, ClientMessage Message)
        {
            uint uId = Message.PopWiredUInt32();
            string badge;
            string desc;
            ...
            using (SqlDatabaseClient MySqlClient = SqlDatabaseManager.GetClient())
            {
                DataRow Description = MySqlClient.ExecuteQueryRow("SELECT * FROM groups_details where id = '" + uId + "'");
                badge = (string)Description["badge"];
                desc = (string)Description["description"];
                name = (string)Description["name"];
                roomid = (int)Description["roomid"];
                members = int.Parse(MySqlClient.ExecuteScalar("SELECT COUNT(*) FROM characters WHERE groupid = '" + uId + "'").ToString());
        ...
        private static void SendGroupAbout(Session Session, ClientMessage Message)
        {
            uint uId = Message.PopWiredUInt32();
            string badge;
            string desc;
        ...
        private static void ActivateGeneric(Session Session, ClientMessage Message)
        {
            ...
            if (Item.Definition.Behavior == ItemBehavior.Switch)
            {
